...
object DataFlowTest2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for ( k <- 0 until maxIterations ) {
      ...
      val mapped2 = mappedRDD.mapPartitions(mapNoOPFun)
      val result = mapped2.reduce(reduceFun)
      sc.broadcast(result)
    }
    println("Total Time for 2" + maxIterations + " : " + (System.currentTimeMillis() - startTime));
  }
  ...
  def mapNoOPFun(iter : Iterator[Array[Double]]) : Iterator[Array[Double]] = {
    iter
  }
}
